...
class Pages extends CI_Controller {
	...
	public function Proyek($params = null, $subparams = null){
		...
		if (in_array($params, $urlArray) && $params != null) {
			...
			if ($params == 'list-proyek') {
				...
				if ($this->input->post('submit')) {
					$insert = $this->db->insert('proyek_realestat', array(
						...
						'tahun_dimulai' => $this->input->post('tahun_dimulai'),
			...
			}else if ($params == 'perijinan') {
				$data['getData'] = $this->db->query('SELECT DISTINCT id_proyek_realestat, id_perusahaan FROM izin_prinsip WHERE id_perusahaan = "'.$cekUser->first_row()->id_perusahaan.'"');
				...
				if (isset($_GET['type']) && $_GET['type'] == 'detail'){
					$data['getDetail'] = $this->db->get_where('izin_prinsip', array(
						'id_perusahaan' => $_GET['ip'],
						'id_proyek_realestat' => $_GET['q'],
				...
				$data['getData'] = $this->db->query('SELECT DISTINCT id_proyek_realestat, id_perusahaan FROM data_fisik WHERE id_perusahaan = "'.$cekUser->first_row()->id_perusahaan.'"');
				...
				// detail
				if (isset($_GET['type']) && $_GET['type'] == 'detail'){
					$data['getDetail'] = $this->db->get_where('data_fisik', array(
						'id_perusahaan' => $_GET['ip'],
						'id_proyek_realestat' => $_GET['q'],
	...
	private function Edit_profile(){
		...
		if ($cekPro->num_rows() == 1 ) {
			...
			if ($this->upload->do_upload()) {
				...
				unlink("avatar/".$cekPro->first_row()->avatar);
	...
	public function resize($path,$file){
		...
		$config['width'] = 500;
		$config['height'] = 500;
		$config["image_sizes"]["square"] = array(500, 500);
	...
	public function Mail(){
		...
		$this->email->from('riski.rinaldi@yahoo.com', 'Rizky RInaldi');
		$this->email->to('rizkyrnldy@gmail.com'); 
