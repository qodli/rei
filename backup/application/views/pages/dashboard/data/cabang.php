                        ...
                        <table class="table table-striped table-bordered dataTable table-tools" id="zero-configuration">
                            ...
                            <tbody>
                               ...
                               <tr>
                                    ...
                                    <td><?php echo $vdata['nama_cabang'] ?></td>
                                    <td><?php echo $vdata['alamat_cabang'] ?></td>
                                    <td><?php echo $vdata['npwp_cabang'] ?></td>
                                    <td><?php echo $vdata['website_cabang'] ?></td>
                                    <td><?php echo $vdata['email_cabang'] ?></td>
                                    <td><?php echo $vdata['telp_cabang'] ?></td>
                                    <td><?php echo $vdata['fax_cabang'] ?></td>
                                    <td><?php echo $vdata['bid_usaha_cabang'] ?></td>
                                    <td>
                                        <select name="" class="form-control" onchange="location = this.options[this.selectedIndex].value;">
                                            <option value="">-- Action --</option>
                                            <option value="<?php echo base_url('data'); ?>/<?php echo $this->uri->segment(2) ?>/add?type=edit&id=<?php echo $vdata['id_cabang'] ?>&ip=<?php echo $vdata['id_perusahaan'] ?>&m=<?php echo base64_encode($this->session->userdata("email")) ?>">Edit</option>
                                            <option value="<?php echo base_url('data'); ?>/<?php echo $this->uri->segment(2) ?>/add?type=delete&id=<?php echo $vdata['id_cabang'] ?>&ip=<?php echo $vdata['id_perusahaan'] ?>">Delete</option>
                                        </select>
                                    </td>
                                </tr>
                            <?php $no++; endforeach; ?>
