		                        ...
		                        <tbody>
		                            <?php $no = 1; foreach ($getData->result_array() as $vdata): ?>
		                                <tr>
		                                    ...
		                                    <td><?php echo $vdata['nama_pemegang_saham'] ?></td>
		                                    <td>
		                                        <select name="" class="form-control" onchange="location = this.options[this.selectedIndex].value;">
		                                            <option value="">-- Action --</option>
		                                            <option value="<?php echo base_url('data'); ?>/<?php echo $this->uri->segment(2) ?>/add?type=edit&id=<?php echo $vdata['id_pemegang_saham'] ?>&ip=<?php echo $vdata['id_perusahaan'] ?>&m=<?php echo base64_encode($this->session->userdata("email")) ?>">Edit</option>
		                                            <option value="<?php echo base_url('data'); ?>/<?php echo $this->uri->segment(2) ?>/add?type=delete&id=<?php echo $vdata['id_pemegang_saham'] ?>&ip=<?php echo $vdata['id_perusahaan'] ?>">Delete</option>
		                                        </select>
		                                    </td>
		                                </tr>
		                            <?php $no++; endforeach; ?>
